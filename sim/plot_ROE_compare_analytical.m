...
%   tstart, tint, tend : simulation start time, timestep, and end time [s]
    ...
    for deputy_idx = 1:length(all_deputies)
        ...
        for init_idx = 1:length(cases_to_plot)
            ...
            d_e_x_mean = roe_results.(init_case).with_j2.(deputy_name).d_e_x_mean;
            d_e_y_mean = roe_results.(init_case).with_j2.(deputy_name).d_e_y_mean;
            d_i_x_mean = roe_results.(init_case).with_j2.(deputy_name).d_i_x_mean;
            d_i_y_mean = roe_results.(init_case).with_j2.(deputy_name).d_i_y_mean;
            d_lambda_mean = roe_results.(init_case).with_j2.(deputy_name).d_lambda_mean;
            d_a_mean = roe_results.(init_case).with_j2.(deputy_name).d_a_mean;
            ...
            % Plot
            h = plot(ax1, d_e_x_mean, d_e_y_mean, 'DisplayName', 'Propagated ecc');
            plot(ax1, d_e_x_analytical, d_e_y_analytical, 'DisplayName', 'Analytical ecc');
            plot(ax2, d_i_x_mean, d_i_y_mean, 'DisplayName', 'Propagated inclination');
            plot(ax2, d_i_x_analytical, d_i_y_analytical,'DisplayName', 'Analytical inclination');
            plot(ax3, d_lambda_mean, d_a_mean, 'DisplayName', 'Propagated mean longitude');
            plot(ax3, d_lambda_analytical, d_a_analytical, 'DisplayName', 'Analytical mean longitude');
        end
